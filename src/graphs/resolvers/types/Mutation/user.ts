...
export default {
  async novoUsuario(_, { dados: { name, email, password, perfil }}) {
    ...
    const already = await db(table_users).where({
      email
    }).first().select('id')
    ...
    const id = await db(table_users)
      .insert(novo) as number

    const result = await db(table_users).where({
      id
    }).first()
  ...
  async excluirUsuario(_, { id }, ctx?: TContext) {
    ...
    const rows = await db(table_users)
    .where('id', id)
    .del()
