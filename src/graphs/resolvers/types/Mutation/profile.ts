...
export default {
  async novoPerfil(_, { dados: { label, name } }, ctx?: TContext) {
    ...
    const already = await db(table_profiles).where({
      label
    }).first().select('id')
    ...
    const id = await db(table_profiles)
      .insert(novo)

    const result = await db(table_profiles).where({
      id
    }).first()
  ...
  async excluirPerfil(_, { id }, ctx?: TContext) {
    ...
    const rows = await db(table_profiles)
    .where('id', id)
    .del()
